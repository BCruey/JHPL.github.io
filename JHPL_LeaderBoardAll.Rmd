...
Leader.Table<-DT::datatable(df,colnames = c('Name', 'Player', 'Games Played', 'Cashes', 'Bubble','KOs','Top 5 Points','Points'),extensions = 'Responsive',options = list(pageLength = 10, responsive=TRUE,
      dom = 'Bfrtip',
      buttons = I('colvis'), columnDefs = list(
    list(
      targets = c(1), # Index of columns to hide
      visible = TRUE,
      responsivePriority = 1
    ),
    list(
      targets = c(-1,-2), # Index of columns to hide
      visible = TRUE,
...
Leader.Table<-DT::datatable(df,colnames = c('Name', 'Player', 'Paid','Bounties Won','Tournament Winnings','ITM','ROI'),extensions = 'Responsive',options = list(pageLength = 10, responsive=TRUE,
      dom = 'Bfrtip',
      buttons = I('colvis'), columnDefs = list(
    list(
      targets = c(1), # Index of columns to hide
    ...
    list(
      targets = c(-3,-2), # Index of columns to hide
      visible = TRUE,
      responsivePriority = 2
    )
  )))%>%
...
MainEvents <- as.Date(c("2024-07-11","2024-08-29","2024-09-28"))
DeepStack <- as.Date(c("2024-08-11","2024-09-08"))


Days_elapsed <- yday("2024-06-30")
...
calendR(year = 2024,from=as.Date("2024-07-01"), to=as.Date("2024-09-30"),
        special.days = Events,
